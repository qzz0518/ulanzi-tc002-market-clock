...
| 能力 | 说明 |
| --- | --- |
| 原生音频播放（网易云） | 服务端代理网易云音频 → 设备下载到 `/tmp/track.mp3` → `base::MediaPlayer` 播放，支持毫秒级 `seekTo` |
| 远程歌词模式（Spotify） | 音频在 Spotify Connect 设备上播放，固件不下载任何音频：歌词时钟跟随服务端上报的播放位置（漂移 > 0.9s 才校正，其余由本地 60ms tick 补帧） |
...
| 实体按键 | 中键：播放/暂停；旋钮按下：循环切换显示形式。左右键与旋钮旋转随音源变化——网易云下是上一句/下一句歌词与本机音量；Spotify 下是上一首/下一首与 Connect 设备音量 |
...
1. **控制轮询**（每 2s）：`GET /api/music/device/state`，纯文本 `KEY\tVALUE`：
   ...
   | `SRC` | 当前音源 `netease` / `spotify` |
   | `RMT` | `1` 表示远程模式（音频在别处播放），此时固件不下载音频 |
   | `TID` | 当前曲目 ID；变化时拉歌词，并在非远程模式下载 `/api/music/device/audio`。网易云是十进制、Spotify 是 base62，固件按不透明字符串处理 |
   | `PLAY` | `1/0` 播放或暂停 |
   | `MODE` | `ticker/skyline/spotlight/cascade` 显示形式 |
   | `SKIN` | `signal/tape/blueprint/arcade` 配色 |
   | `ACCENT` | 覆盖主色的 `rrggbb`，`-` 表示跟随配色 |
   | `SEEK` | 目标毫秒；固件按值去重，服务端换曲时重置为 `-1`（仅非远程模式） |
   | `RPOS` / `RDUR` / `RPLAY` / `RVOL` | 远程播放器的位置、时长、播放状态与音量；`RMT=1` 时有效，`-1` 表示未知 |

   远程模式下 `RPOS` / `RPLAY` 每轮都读（不看 `SEQ`）：Connect 播放器的位置自己在走，
   没人会为它递增 `SEQ`。
...
2. **心跳上报**（同一轮询循环内、选定曲目后开始）：
   ...
   `trackId` 以 JSON 字符串上报（Spotify ID 不是数字），服务端同时兼容旧固件的裸数字。
...
播放/暂停、旋钮按下的主题切换）；网易云模式下左右键跳句不单独上报，由下一次心跳的
`playheadMs` 回流到网页，音量属于设备本地状态、不回传。

远程模式下 `report` 同时**就是控制命令**——设备没有本地音频可暂停，服务端会把它
翻译成 Spotify Connect 调用：中键 `{"playing":…}` → play/pause，左右键
`{"action":"previous"|"next"}` → 切歌，旋钮 `{"volume":0-100}` → 设备音量。
