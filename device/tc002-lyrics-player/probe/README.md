...
验证整条非持久化会话链路：ADB 推送 → tmpfs 解包 → `ctl.stop zkswe` 暂停官方
...
```bash
COPYFILE_DISABLE=1 tar -czf /tmp/player.tar.gz -C device/tc002-lyrics-player/probe player
bun run music-release -- /tmp/player.tar.gz 0.0.1-probe player
```
