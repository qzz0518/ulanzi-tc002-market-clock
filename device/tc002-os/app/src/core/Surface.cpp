...
void Surface::fill(const Color& color) {
    // The buffer is tightly packed RGB (3 bytes per pixel). The upstream copy
    // indexed with i*4 here, writing ~830 bytes past the allocation.
    for (int i = 0; i < mWidth * mHeight; i++) {
        mData[i * 3 + 0] = color.r;
        mData[i * 3 + 1] = color.g;
        mData[i * 3 + 2] = color.b;
...
Color Surface::getPixel(int x, int y) const {
    ...
    int index = (y * mWidth + x) * 3;
    return Color(mData[index + 0], mData[index + 1], mData[index + 2]);
}
