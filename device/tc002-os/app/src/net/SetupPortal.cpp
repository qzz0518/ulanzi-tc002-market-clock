...
const char* kPageHtml =
    ...
    "o.textContent='正在扫描…';s.appendChild(o);setTimeout(load,2000);return}"
    "d.networks.forEach(function(n){var o=document.createElement('option');"
    "o.value=n;o.textContent=n;s.appendChild(o)})})}"
    "function go(){var b=document.getElementById('b');b.disabled=true;show('正在连接…');"
    "fetch('/connect',{method:'POST',headers:{'Content-Type':"
    "'application/x-www-form-urlencoded'},body:'ssid='+encodeURIComponent("
    "document.getElementById('s').value)+'&password='+encodeURIComponent("
    "document.getElementById('p').value)}).then(function(){poll()})}"
    "function poll(){fetch('/status').then(r=>r.json()).then(function(d){"
    "if(d.status=='online'){show('已连接，设备地址 '+d.ip+'。可以关闭本页。');return}"
    "if(d.status=='failed'){show('连接失败，请检查密码后重试。');"
