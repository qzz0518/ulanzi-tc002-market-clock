...
> **设备端放音已写好但未经真机验证**（设置 → 音乐播放，默认「控制台」），见「音乐与设备放音」。
...
音乐页默认是一块由控制台喂的歌词显示器：网易云的声音从浏览器那台电脑出，Spotify 从 Connect
设备出，面板只画服务端拉取文档里的当前曲目与歌词。**设置 → 音乐播放** 切到「时钟」之后，
网易云改由时钟自己出声（`net/DeviceAudio` + `platform/TrackPlayer`）：

- 走的是**侧载播放器那套协议**，服务端零改动：每 2 s 轮询 `/api/music/device/state`，曲目变了就
  把 `/api/music/device/audio` 下载到 `/tmp/track.mp3`（`HttpClient::streamGet` 落盘，
  `.part` 收全再改名，上限 24 MB），交给 SDK 的 `base::MediaPlayer` 解码到 MI_AO；载入后每轮
  心跳 `/api/music/device/heartbeat`。浏览器看到心跳就闭嘴，变成静默遥控，跟着心跳里的播放头
  上报歌词——这一段控制台行为是侧载播放器时代真机验证过的。
- **两个身份**：轮询带 `?viewer=zos`，服务端不记成 `FWPOLL`（那是控制台「侧载占着设备、锁其他
  视图」的依据，ZOS 下频道与镜像照常）；只有心跳宣告「我是播放器」，且只在真的载入曲目、音源不是
  Spotify 时发。开关关掉，心跳停，十秒后浏览器重新接管。
- 中键在时钟放歌时**就地**切播放 / 暂停并上报 `{"playing":…}`，而不是发一条让控制台执行的命令
  ——它现在是遥控，能做的只有反过来问我们。旋钮上下曲仍发给控制台，队列在浏览器里。
- 播放头是本地时钟（载入归零、seek 重锚、暂停冻结），不读 `MediaPlayer::currentPositon()`：
  侧载播放器也从没读过它，浏览器那边的歌词时序是照着这种时钟调的。
- 全部决策（首个文档、同序列重复、暂停、seek、下载中换曲、失败 10 s 后重试、远程音源、
  本地按键与服务端回声不互相翻转）都是纯函数返回的 `Plan`，`hostcheck/selfcheck.cpp`
  `checkDeviceAudio` 逐条断言；设备侧只剩五个转发调用和一个播完回调。

**默认关，且开关在设备上**，因为这条路一旦在某台机器上坏掉（解码器不认文件、输出被音效混音器
占着），浏览器就是唯一另一个喇叭，而正是时钟的心跳让浏览器闭嘴——默认开等于一个坏 build
让设备彻底没声音。故障在时钟上观察到，开关就放在时钟上。**这条路尚未在真机跑过**：解码器
与 `Sfx` 的 `AudioPlayer` 是否抢同一路输出、下载 + 解码在 36 MB 内存里的余量、`seekTo`
在暂停态是否生效，都要台架上见。
...
它也能**自己翻**：控制台发下来的 `vibeauto`（秒，`0` = 只认旋钮）到点就转一格。计时锚是独立于
`mPageMs` 的 `mAutoAtMs`——转旋钮或按一下都重新计时，但不把数值/倒计时的相位一起拨回去；间隔本身
变了也重新计时，否则一个在这页站了四分钟的人会在控制台按下保存的下一帧就被翻走。距上次计时超过
两个间隔视为「刚从黑屏回来」（夜间息屏整夜不 render），重新计时而不翻页，免得面板一亮就已经不是
用户离开时那页。这不是一次按键，所以不响旋钮的提示音。
...
翻页为锚，所以走进一页永远先看到数字。这两段各占多久由控制台的 `vibedwell` 说了算（毫秒，出厂
3200 / 1600），倒计时那半发 `0` 就是「只显示数值」；服务不发这个键时两个默认值照旧。
...
线协议是六个新键（`vibe` / `vibeauto` / `vibedwell` / `vibea` / `vibes` / `vibem`），全部新增、不给旧键加字段——已部署固件
...
音乐照放（音效的 `AudioPlayer` 与放歌的 `MediaPlayer` 都不在渲染路径上）。醒来那一帧 `dt` 被夹在 250 ms，所以八小时空档不会
...
- **体积上限 2,600,000 字节。** 这个数字在 ADR 0014 之前是 1,400,000，当时它只为拦一件事：
  ...
  解复用器、解码器和它们的表共 945,804 字节。上限还要拦的是：没 strip 的 .so（ffmpeg 之前就是
  ~6.7 MB），以及第二条误入的解码路径（同一批库的视频侧 swscale / avfilter 又是一兆，而这里
  不需要 ffmpeg 的任何一个像素）。2,600,000 在当前二进制之上约 300 KB——**文件大小只按 4,096
  字节一跳**（链接器给只读段补页，1.2 MiB 时代就是这么被 965 字节撞上的），也就是 73 档。
  它不再承诺的是 tmpfs 那条论据：这台机器被侧载过的最大 .so 是音乐固件的 1,840,452 字节，
  现在比它大 25%。刷进 `res`（见「固化到 flash」）是从 squashfs mmap，被替掉的官方 app 是
  7,464,044 字节，闪存不是问题；**把这个 build 侧载进 `/tmp` 是唯一没人跑过的配置。**
