...
![音乐工作台：搜索选歌、主题设置与 52×16 实时像素预览](docs/images/tc002-music-studio.png)

顶部「音乐」是一个完整的音乐工作台：网易云扫码登录、搜索（20 首/页）、歌单、逐行歌词
（含翻译）、同源音频代理和 52×16 像素歌词实时预览。登录 Cookie 只存本机
...
（信号绿 / 磁带橙 / 蓝晒 / 街机红），另可用取色器覆盖自定义主色。

两条互补的上屏路径：

- **设备同屏（官方固件，不刷机）**：把渲染好的歌词帧（≤60 帧、约 15fps）推到官方固件的
  Custom App 通道显示；声音由浏览器播放。
- **原生音乐固件（非持久化旁载）**：仓库内含完整的 FlyThings C++ 播放器（Docker 交叉
  编译，无需 Windows IDE），在真机上下载音频、扬声器播放、毫秒级进度跳转，并用离线
  光栅化的 12×12 中日文字模直接驱动 LED。网页与固件通过控制序列 + 心跳协议**双向实时
  同步**：网页选歌/暂停/换主题/拖进度 2 秒内落到设备，设备按键即时回流，预览动画锚定
  真机播放头；此模式下网页静音、只作遥控，固件在线时界面自动切换。官方固件没有能解码
  网络音频的播放器，扬声器出声必须由设备端应用调用 `AudioManager`——这正是旁载固件
  存在的原因（MQTT 只能传控制消息，替代不了它）。

<p align="center">
  <img src="docs/images/tc002-music-firmware-preview.png" width="720" alt="52×16 像素歌词屏——预览与音乐固件使用同一套渲染算法">
</p>

旁载始终是非持久化的：会话只把播放器推进设备内存盘临时运行，结束会话或断电重启即恢复
官方固件，flash 从不被写入。启动会话前需三重确认：旁载包逐文件 SHA-256 与发布清单一致、
官方 HTTP 接口与 Wi-Fi ADB 双重确认真机、用户勾选已知恢复方式。固件源码、协议、构建与
