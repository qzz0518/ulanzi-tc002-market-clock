...
| 视觉 | 兰顿蚂蚁、鱼缸、火焰、翻页钟、数字雨时钟、走迷宫、像素宠物、落沙、星空穿梭、生命游戏、烟花、天气粒子、日出日落色温钟（天气类按地名搜索定位，免填坐标） |
| 创作 | 52×16 画板（画笔、像素文字、图片像素化，可开直播上屏 + 扫码协作涂鸦）；素材库导入 Ulanzi 官方社区素材（PNG / GIF）或本地视频（ffmpeg 转 52×16 像素动画） |

控制台右上角可直接读写时钟的亮度、音量、时区等常规设置；「设备信息」页显示 SN、SSID、
IP、MAC 与固件版本，时钟换了 IP 也能在这里直接改，立即生效、重启后仍然有效，不用重装。
手机浏览器打开有专门的触控布局，支持添加到主屏幕。
...
顶部「游戏」是一个像素游戏厅：**时间打砖块、像素小鸟、贪吃蛇、双人 Pong** 在浏览器里
直接玩（触控 / 鼠标 / 键盘），打开「上屏」后画面以约 25fps 实时同步到时钟——不刷机，
时钟就是第二块屏。双人 Pong 可以扫码把手机变成第二个手柄。

想脱离浏览器、用时钟自己的旋钮和按键玩，就侧载仓库自带的**游戏固件**：开机动画、
卡带式游戏菜单、设备信息页（电量 / USB / 版本 / IP，旋钮实时调音量），**七款游戏**
（上述四款外加极速车道、太空射击、横版俄罗斯方块）配 8-bit 音效。侧载方式与音乐固件
完全相同：只进设备内存、断电或一键恢复即回到官方固件，flash 从不被写入。
