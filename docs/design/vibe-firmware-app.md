...
- 状态:已实施(2026-08-14 真机验证:侧载新固件后根环出现 VIBE,各代理数字正常)
- 日期:2026-08-14
- 设计:Claude Fable
...
- **频道与用户内容争位置**。四个代理会把用户自己的行情、时钟挤出那个一次只显示一项的环。
...
体积:四个代理各两行指标 ≈ 四行 `vibea` + 四行 `vibes` + 八行 `vibem` ≈ 0.7 KB,
相对文档现有体量可忽略。(原为七个代理约 1.2 KB;每行约 43 B,行数从 28 降到 16。)
...
页 1..N:每个代理一页(16×16 彩色图标 + ≤2 条指标 + 进度条 + 重置倒计时)
```
...
- 详情:**16×16 彩色图标(x=0..15、y=0..15,1:1 不缩放)**+ 行区 x=17..51(35px),
  行 1 y=2..6 / 行 2 y=9..13;单字符指标标签(x=17)+ 14px 进度条(x=21..34)+ 右对齐数值
  (至 x=51)。行区整体较 12px 图标那版右移 2px,行内间距不变;x=16 是唯一的隔离列。
- 严重度配色沿用 80%/90% 两档(蓝/琥珀/红),与控制台、LED 一致。
- `stale` 的代理在右上角点一颗琥珀像素——与频道版同一约定。

### 4.3 图标:一份位图,两处消费

设备要画四个厂商的 12×12 标记。仓库已有先例:CJK 字模由离线脚本同时喂固件头文件与网页预览,
...
**16×16 彩色标记(后续)**:详情页改画 `src/vibe/vibe-pixel-logos.ts` 的手绘 16×16 品牌图。
面板正好 16 行,所以 1:1 直出,不缩放——那份美术的 README 明令禁止非整数倍重采样,而 16→10
恰恰不是整数倍,所以总览页原样保留单色 `s10`。同一个生成脚本多出一个 `ColorMark` 数组:透明 +
至多三色 = 2 bit/px,一行一 uint32(`bit31..30` 为最左),每图标 64 字节点阵 + 9 字节调色板,
四个图标共 292 字节。索引 0 是透明,不落笔,页面从它后面透出来。
`drawColorMark()` **不收颜色参数**:这些是采样自品牌的颜色,用页面的严重度色盖掉它,和给照片
上色是同一种错误。没有 16×16 图的厂商(含中性 `gauge`)回落到单色 12px 标记,居中于同一列。
...
4. 真机:根环出现「VIBE」,进去能翻页看到各代理的真实数字,长按返回。
