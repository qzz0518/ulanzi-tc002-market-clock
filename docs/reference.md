...
| `POST` | `/api/os/firmware/restore` | 把**官方固件还原点**放进待装位（同源检查，正文 `{}`）。读的是组合根里写死的 `.runtime/tc002-stock/restore-live.img`——路径不是入参，因为这条路决定往闪存里写哪堆字节，让调用方指定文件等于让调用方指定刷什么。**走的是和上传完全同一条校验**（`storeUpload`），所以一份在盘上烂掉的还原点会在这里响亮地失败，而不是等设备已经擦掉 mtd3 才发现。没配还原点 404，配了但文件不在 409（并说明它无法再生成），容器不合法 400。通过则应答与状态接口同一份文档，且 `source.kind` 是 `upload`——所以「移除上传」照样能把 ZOS 换回来，装填还原点不是单向门。**同样只是装填，不安装**：安装仍是 `/api/os/upgrade` 那一次单独的、单独同意过的动作 |
...
| `GET` | `/api/os/firmware/status` | 控制台读镜像状态：`{packed, image, source, shadowedPacked, restore, upgradeSeq}`。`image` 是 `{bytes, buildId, builtAt, md5, partitionType, partitionLabel, payloadBytes, zosBuildId, filesystemBuiltAt}` 或 `null`——`md5` 是**整包**的摘要（主人对着自己的文件跑一次 `md5` 就能核），`buildId` 是设备侧校验的那份内嵌 MD5，两者不是一个数。`zosBuildId` 只有在载荷里真的读得到那串 `<rev>-<stamp>` 时才有值，打包镜像里它压在 xz squashfs 底下，所以正常就是 `null`，控制台照直写「未知」——**不从大小、mtime 或摘要里凑一个像版本号的东西出来**。`filesystemBuiltAt` 是 squashfs 的 mkfs 时间，打包器把它钉在所打的 `.so` 的 mtime 上，是唯一一个扛得住 xz 的「这是哪一版」线索。`source` 是 `{kind:"upload"\|"packed", fileName, at}`：分不清「这是我刚打的」和「这是别人给我的文件」，正是照着昨天的构建装完还以为装了今天那一版的原因。`shadowedPacked` 是**存在但不会被安装**的那份本地打包镜像（有上传时才非空）。`restore` 是官方固件还原点：`{available, path, bytes, builtAt}`，服务端太老则为 `null`——「没配置还原」和「配了但文件不在」是两件事，控制台对它们说的话不一样。`builtAt` 是选中镜像的 mtime；`packed` 明写而不靠字段有无去猜——它旁边就是那颗改写闪存的按钮 |
